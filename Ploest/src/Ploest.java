...
public class Ploest {
	...
	public static void printHelp() {
		...
		System.out.println("\nPedca -help:");
		System.out.println(
				"\nUSAGE:    java –jar Pedca.jar  -p <projectname> -i < input sam/bam File> -o <outputFolder>  <<OPTIONAL PARAMETERS>> \n");
		System.out.println("REQUIRED PARAMETERS:");
		System.out.println("-p (projectname)          – (String)  Prefix used to generate the results file.");
		System.out.println(
				"-i (inputfile)            - (Pathway to .bam/.sam file)   Pathway (absolute if necessary) to the input file containing the alignment file. Must be a .bam or .sam file");
		System.out.println(	"-o (outputFolder)         - (String)   Pathway (absolute if necessary) to the autogenerated output folder that will contain the results('./' if the folder points to the current directory)");
		...
		System.out.println(
				"-s (significant min)      - (double) Threshold to consider a cluster peak in the read count to be sigificant. Default 0.1");
		System.out.println(
				"-b (fitter bin size)      - (int)  The size of the bin used to FIT the read count distribution");
		System.out.println(
